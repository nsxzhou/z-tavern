...
func NewFullClientRequest() []byte {
	...
	request.Write(DefaultHeader().WithMessageTypeSpecificFlags(common.NO_SEQUENCE).toBytes())
...
func NewAudioOnlyRequest(seq int, segment []byte) []byte {
	...
	header.WithSerializationType(common.NO_SERIALIZATION)
	if seq < 0 {
